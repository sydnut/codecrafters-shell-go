...
func main() {
	...
	for {
		...
		case '\t':
			...
			prompt, isFile := parse4AutoCompletion(input)
			var beforeAC string
			//mark
			if i := strings.LastIndex(input, prompt); i != -1 {
				beforeAC = input[:i]
			}
			if !isFile {
				...
				if tmp, _ := acRecord[prompt]; len(cmds) > 1 && !isLcp && !tmp {
					...
					//第二次触发
					fmt.Printf("\r\n%s\r\n$ %s%s", strings.Join(cmds, "  "), beforeAC, prompt)
					acRecord[prompt] = false
					continue
				} else if isLcp && len(cmds) > 1 {
					if t, _ := acRecord[prompt]; !t {
						fmt.Printf("\x07")
						acRecord[prompt] = true
						inputBuf.Reset()
						inputBuf.WriteString(beforeAC)
						inputBuf.WriteString(lcp)
						fmt.Printf("\r$ %s%s", beforeAC, lcp)
						continue
					} else {
						fmt.Printf("\r\n%s\r\n$ %s%s", strings.Join(cmds, "  "), beforeAC, prompt)
						acRecord[prompt] = false
				...
				//只显示第一个
				fmt.Printf("\r$ %s%s ", beforeAC, cmds[0])
				inputBuf.Reset()
				inputBuf.WriteString(beforeAC)
				...
				if strings.Contains(prompt, "/") {
					...
					//切换目录树
					//手动分割
					var bytes []byte = []byte(prompt)
					var idx int
					for idx = len(bytes) - 1; idx >= 0 && bytes[idx] != '/'; idx-- {
					}
					dir, filePrefix := string(bytes[:idx]), string(bytes[idx+1:])
					//父路径
					var parentDir string = strings.Join([]string{nowDir, dir}, "/")
					workingDirTrie, err = newFileTrie(parentDir)
					//todo 先暂时暴力处理异常
					if err != nil {
						fmt.Fprintf(os.Stderr, "%v\n", err)
						os.Exit(-2)
					}
					exist, files := workingDirTrie.startWith(filePrefix)
					if !exist {
						fmt.Printf("\x07")
						continue
					}
					isLcp, lcp := lcp(files)
					if tmp, _ := acRecord[prompt]; len(files) > 1 && !isLcp && !tmp {
						//第一次触发响铃
						fmt.Printf("\x07")
						acRecord[prompt] = true
						workingDirTrie = temp
						...
						//第二次触发
						fmt.Printf("\r\n%s\r\n$ %s%s", strings.Join(files, "  "), beforeAC, prompt)
						acRecord[prompt] = false
						workingDirTrie = temp
						continue
					} else if isLcp && len(files) > 1 {
						if t, _ := acRecord[strings.Join([]string{dir, prompt}, "/")]; !t {
							fmt.Printf("\x07")
							acRecord[strings.Join([]string{dir, prompt}, "/")] = true
							fmt.Printf("\r$ %s%s", beforeAC, strings.Join([]string{dir, lcp}, "/"))
							inputBuf.Reset()
							inputBuf.WriteString(beforeAC)
							inputBuf.WriteString(strings.Join([]string{dir, lcp}, "/"))
						} else {
							fmt.Printf("\r\n%s\r\n$ %s%s", strings.Join(files, "  "), beforeAC, prompt)
							acRecord[prompt] = false
						}
					...
					//只显示第一个(文件)
					fmt.Printf("\r$ %s%s", beforeAC, strings.Join([]string{dir, files[0]}, "/"))
					inputBuf.Reset()
					inputBuf.WriteString(beforeAC)
					inputBuf.WriteString(strings.Join([]string{dir, files[0]}, "/"))
					if info, _ := os.Stat(strings.Join([]string{parentDir, files[0]}, "/")); !info.IsDir() {
						inputBuf.WriteByte(' ')
						fmt.Print(" ")
					}
					//最后恢复
					...
					//todo 以及看是不是目录决定补全内容,同时复用下面到上面
					exist, files := workingDirTrie.startWith(prompt)
					if !exist {
						fmt.Printf("\x07")
						continue
					}
					...
					//只显示第一个(文件)
					fmt.Printf("\r$ %s%s", beforeAC, files[0])
					inputBuf.Reset()
					inputBuf.WriteString(beforeAC)
					inputBuf.WriteString(files[0])
					//todo 暂时不考虑异常
					if info, _ := os.Stat(nowDir + "/" + files[0]); !info.IsDir() {
						inputBuf.WriteByte(' ')
						fmt.Print(" ")
					}
				}
		...
		case '\r', '\n':
			...
			inputBuf.Reset()
			//重置补全错误标记
			for k := range acRecord {
				delete(acRecord, k)
			}
			//如果工作路径改变记得切换工作路径补全树
			if workingDirTrie.name != nowDir {
				workingDirTrie, err = newFileTrie(nowDir)
			}
		case 0x03:
			//ctrl-c
			_ = term.Restore(fd, oldTerminalState)
...
func initEnv() {
	...
	initNowDir()
	//初始化工作路径补全树
	workingDirTrie, _ = newFileTrie(nowDir)
}
...
func lcp(strs []string) (bool, string) {
	if len(strs) <= 1 {
		return false, ""
	}
	model := strs[0]
