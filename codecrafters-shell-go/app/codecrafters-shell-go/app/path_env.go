...
/*
parse the input and return the word to be auto-completed
return false if it's command-auto-completion
*/
func parse4AutoCompletion(input string) (ret string, isFile bool) {
	var res []string = make([]string, 0)
	var buf strings.Builder
	var (
		hasSinQuote bool = false
	...
	for i := 0; i < len(input); {
		...
		if !hasSinQuote && !hasDouQuote {
			...
			if input[i] == '"' {
				...
				// params[i]==\
				if i < len(input) && ((input[i+1] == '"') || (input[i+1] == '\\')) {
					buf.WriteByte(input[i+1])
					i++
				} else {
					buf.WriteByte(input[i])
